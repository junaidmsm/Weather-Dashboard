...
searchButton.submit(function (event) {
	...
	if (searchInput == "") {
		...
		$.ajax({
			...
			var fiveDayCard = $("<div>").addClass("row");
			for (let i = 0; i < data.length; i = i + 8) {
				...
				fiveDayCard.append("<div class='card-body size m-3 p-3'><div class=fiveDayColor>" + "<p>" + FiveDayTimeUTC1 + "</p>" + `<img src="https://openweathermap.org/img/wn/${response.list[i].weather[0].icon}@2x.png">` + "<p>" + "Temperature: " + response.list[i].main.temp + "</p>" + "<p>" + "Humidity: " + response.list[i].main.humidity + "%" + "</p>" + "</div></div>");


			}
			$("#five-day").html(fiveDayCard)
